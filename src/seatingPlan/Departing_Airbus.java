...
public class Departing_Airbus extends JFrame implements ActionListener{
	...
	public ArrayList<JButton> da_seat_selected = new ArrayList();
	
	String a_flight_num;
	
	
	ArrayList<String> d_seats_chosen = new ArrayList();
	...
	public Departing_Airbus(int a_flight_duration, String ticket_num, String d_flight_num, String a_flight_num, 
						String from, String to, String depart_date, String return_date, D_Selected_Seats dss, 
							R_Selected_Seats rss, 	passengerRecord pr, Double d_distance, Double a_distance, Boolean d_isAirbus,
								Boolean a_isAirbus, Double d_flight_distance, Double a_flight_distance, int d_delay) {
		this.from = from;
		this.to = to;
		...
		this.a_flight_duration = a_flight_duration;
		...
		setTitle("Departing- Airbus A318");
	...
	ArrayList<String> flight_number = dss.returnFlightNum();
	ArrayList<ArrayList<JButton>> selected_seats = dss.returnSeats();
	
	for(int i=0; i<btns.length; i++) {
	
		for(int j =0; j<flight_number.size();j++) {
		
			if(flight_number.get(j).equals(d_flight_num)) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		for(int i=0;i<btns.length;i++) {
			...
			if(e.getSource()==btns[i]) {
				...
				btn.remove(btns[i]);
				da_seat_selected.remove(btns[i]);
				btns[i].setBackground(new Color(0x2A9296));
				counter--;
				}
				else {
		...
		if(e.getSource()==done) {
			...
			JOptionPane.showMessageDialog(this, "You must select " + ticket_num + (Integer.valueOf(ticket_num) > 1 ? " Seats.":" Seat."));
				...
				d_btn = (ArrayList<JButton>) btn.clone();
				
				if(a_flight_duration>120) {
					Returning_Boeing m = new Returning_Boeing(a_flight_duration, ticket_num, a_flight_num, from, to, depart_date, 
																return_date, d_btn, d_flight_num, dss, rss, pr, d_isAirbus, a_isAirbus,
																	d_flight_distance, a_flight_distance, d_delay);
					m.setLocationRelativeTo(this);
				}
				else {
					Returning_Airbus m =new Returning_Airbus(a_flight_duration, ticket_num, a_flight_num, from, to, depart_date, return_date, 
																d_btn, d_flight_num, dss, rss, pr, d_isAirbus, a_isAirbus, 
																	d_flight_distance, a_flight_distance, d_delay);
					m.setLocationRelativeTo(this);
				}
