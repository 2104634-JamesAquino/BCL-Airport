...
public class Departing_Boeing extends JFrame implements ActionListener{
	...
	public ArrayList<JButton> db_seat_selected = new ArrayList();
	...
	public Departing_Boeing(int a_flight_duration, String ticket_num, String d_flight_num, String a_flight_num, String from,
						String to, String depart_date, String return_date, D_Selected_Seats dss, R_Selected_Seats rss, 
						 passengerRecord pr, Double d_distance, Double a_distance, Boolean d_isAirbus, Boolean a_isAirbus,
		...
		for(int i=0;i<btns.length;i++) {
			if(e.getSource()==btns[i]) {
				if(btn.contains(btns[i])) {
					btn.remove(btns[i]);
					db_seat_selected.remove(btns[i]);
					btns[i].setBackground(new Color(0x2A9296));
					counter--;
				}
				else {
			...
			if(counter!=Integer.valueOf(ticket_num)) {
				JOptionPane.showMessageDialog(this, "You must select " + ticket_num + (Integer.valueOf(ticket_num) > 1 ? " Seats.":" Seat."));
					...
					if(a_flight_duration>120) {
						...
						a_isAirbus = true;
						Returning_Airbus m =new Returning_Airbus(a_flight_duration, ticket_num, a_flight_num, from, to, depart_date,
																	return_date, d_btn, d_flight_num, dss, rss, pr, d_isAirbus, a_isAirbus,
																	d_flight_distance, a_flight_distance, d_delay);
						m.setLocationRelativeTo(this);
					}
	...
	public ArrayList<String> d_seats() {
		return d_seats_chosen;
	}
